...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def prediction_step(
        ...
        for k in self.model.additional_input_kwargs:
            add_kwargs[k] = inputs[k] if k in inputs.keys() else None
        ...
        gen_text = self.tokenizer.batch_decode(generated_tokens[:1], skip_special_tokens=True)
        input_text = self.tokenizer.batch_decode(inputs["input_ids"][:1], skip_special_tokens=True)
        print("\nGenerated_sents:\n", gen_text[0])
        print("\nInput_sents:\n", input_text[0])
        """
